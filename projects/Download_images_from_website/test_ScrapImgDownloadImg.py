...
def download_img(img_link, index, output):
    try:
...
class Test_ScrapImgDownloadImg:
    ...
    def test_successful_image_download(self):
        ...
        if not os.path.isdir(output_dir):
            os.mkdir(output_dir)
        module.download_img(url, index, output_dir)
        assert os.path.isfile(os.path.join(output_dir, f'{index+1}.jpg'))
        with tempfile.NamedTemporaryFile(delete=False) as tempf:
            tempf.write(requests.get(url).content)
        assert filecmp.cmp(os.path.join(output_dir, f'{index+1}.jpg'), tempf.name)

    @pytest.mark.regression
    def test_non_image_url(self):
        url = 'https://example.com'
        index = 0
        assert not module.download_img(url, index, 'output')
